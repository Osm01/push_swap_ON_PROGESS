...
int max, int max_prev)
{
	...
	button_count_to_max = count_move_to_do(*stack_b) - top_count_to_max + 1;
	top_count_to_max_prev = count_move_to_do_from_top(*stack_b, max_prev);
	button_count_to_max_prev = count_move_to_do(*stack_b) - \
	top_count_to_max_prev + 1;
	if ((top_count_to_max <= (count_elements / 2) && (top_count_to_max <= button_count_to_max)) || \
	(top_count_to_max_prev <= (count_elements / 2) && (top_count_to_max_prev <= button_count_to_max_prev)))
	{
		if (top_count_to_max < top_count_to_max_prev)
			return (move_stack_b_to_push_to_a(stack_b, stack_a, \
			top_count_to_max, button_count_to_max - 1), max);
		else
			return (move_stack_b_to_push_to_a(stack_b, stack_a, \
			top_count_to_max_prev, button_count_to_max_prev - 1), max_prev);
	}
	else
	{
